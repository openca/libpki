...
/*! \brief Generates a new PKI_X509_SCEP message */

PKI_X509_SCEP_MSG *PKI_X509_SCEP_MSG_new ( SCEP_MESSAGE_TYPE type ) {

	PKI_X509_SCEP_MSG * ret = NULL;
...
/*! \brief Frees the memory associated with a PKI_X509_SCEP_MSG */
...
/*! \brief Encodes a SCEP_MSG to a PKCS7 structure */
...
/*! \brief Retrieves the decoded data (raw) from a SCEP_MSG */
...
/*!
 *  \brief Retrieves the X509 object from the SCEP_MSG
 */
...
PKI_X509 *PKI_X509_SCEP_MSG_get_x509_obj ( PKI_X509_SCEP_MSG *msg,
	...
	if((mem = PKI_X509_PKCS7_decode ( msg, key, x )) == NULL ) {
		PKI_log_debug("Can not decode SCEP message");
		return NULL;
	};
...
/*! \brief Add a signer to a SCEP_MSG */
...
/*! \brief Add a signer to a SCEP_MSG by using the passed toke data */

int PKI_X509_SCEP_MSG_add_signer_tk ( PKI_X509_SCEP_MSG *msg,
...
PKI_X509_SCEP_MSG * PKI_X509_SCEP_MSG_new_certreq ( PKI_X509_KEYPAIR *key,
	...
	PKI_X509_SCEP_MSG *ret = NULL;
	PKI_X509_SCEP_DATA *scep_data = NULL;
	...
	if ( !recipients ) {
		PKI_log_err ("Recipients are required to encrypt SCEP messge!");
		return NULL;
	}
	...
	if ( req && req->value ) {
		...
		/* We need the request for the inner P7 (encrypted) */
        	if((my_request = PKI_X509_REQ_new( key, subject, NULL,
                                        NULL, NULL, NULL )) == NULL ) {
                	PKI_log_err( "SCEP_MSG_new_certreq()::Can not generate "
	...
	if ( (scep_data = PKI_X509_SCEP_DATA_new ()) == NULL ) {
		PKI_log_err ( "Memory Failure");
		goto err;
	}

	if ( PKI_X509_SCEP_DATA_set_recipients ( scep_data, 
				recipients ) == PKI_ERR ) {
		PKI_log_err ( "Can not set recipients in SCEP message!");
		goto err;
	}

	if ( PKI_X509_SCEP_DATA_set_x509_obj( scep_data, 
				my_request ) == PKI_ERR ) {
		goto err;
	}
	...
	if(( ret = PKI_X509_SCEP_MSG_new(PKI_X509_SCEP_MSG_PKCSREQ)) == NULL ) {
		PKI_log_err ( "Memory Failure");
		goto err;
	}

	if( PKI_X509_SCEP_MSG_add_signer ( ret, my_signer, key, md ) 
			== PKI_ERR ) {
		PKI_log_err ( "Can not set the SCEP message signer");
		goto err;
	}

	PKI_X509_SCEP_MSG_set_sender_nonce ( ret, NULL );
	PKI_X509_SCEP_MSG_set_type ( ret, PKI_X509_SCEP_MSG_PKCSREQ );

	if (PKI_X509_SCEP_MSG_encode ( ret, scep_data ) == PKI_ERR ) {
		PKI_log_err ( "Can not encode SCEP message!");
		goto err;
	}
...
err:
	...
	if ( scep_data ) PKI_X509_SCEP_DATA_free ( scep_data );
	if ( ret ) PKI_X509_SCEP_MSG_free ( ret );
