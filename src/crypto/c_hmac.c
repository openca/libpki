...
/*
 * \brief Allocates and return a new (empty) CRYPTO_HMAC
 */
CRYPTO_HMAC *PKI_HMAC_new_null(void)
{
	CRYPTO_HMAC *ret = NULL;
	ret = PKI_Malloc(sizeof(CRYPTO_HMAC));
...
/*
 * \brief Allocates and initializes a new CRYPTO_HMAC
 */
CRYPTO_HMAC *PKI_HMAC_new(unsigned char *key, size_t key_size, PKI_DIGEST_ALG *digest, HSM *hsm)
{
	CRYPTO_HMAC *ret = PKI_HMAC_new_null();
	if (!ret) return ret;
...
/*
 * \brief Allocates and initializes a new CRYPTO_HMAC by using a PKI_MEM to hold the secret key
 */
CRYPTO_HMAC *PKI_HMAC_new_mem(PKI_MEM *key, PKI_DIGEST_ALG *digest, HSM *hsm)
{
	CRYPTO_HMAC *ret = NULL;
...
/*
 * \brief Frees the memory associated with a CRYPTO_HMAC
 */
void PKI_HMAC_free(CRYPTO_HMAC *hmac)
{
	if (!hmac) return;
 ...
 */
int PKI_HMAC_init(CRYPTO_HMAC *hmac, unsigned char *key, size_t key_size, PKI_DIGEST_ALG *digest, HSM *hsm)
{
...
int PKI_HMAC_update(CRYPTO_HMAC *hmac, unsigned char *data, size_t data_size)
{
	...
	{
		return PKI_ERROR(PKI_ERR_GENERAL, "CRYPTO_HMAC is not initialized");
	}
...
int PKI_HMAC_update_mem(CRYPTO_HMAC *hmac, PKI_MEM *data)
{
...
int PKI_HMAC_finalize(CRYPTO_HMAC *hmac)
{
	int size = 0;
 ...
 */
PKI_MEM * PKI_HMAC_get_value(CRYPTO_HMAC *hmac)
{
	if (!hmac) return NULL;
 ...
 */
PKI_MEM *PKI_HMAC_get_value_b64(CRYPTO_HMAC *hmac)
{
	PKI_MEM *ret = NULL;
