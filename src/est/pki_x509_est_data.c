...
/*! \brief Generates a new EST_DATA */
...
/*! \brief Frees the memory associated with a PKI_X509_EST_DATA */
...
/*! \brief Adds a recipient to a EST_DATA */
...
/*! \brief Adds a stack of recipients for a EST_DATA */
...
/*! \brief Sets the content of the EST_DATA via a PKI_X509 object */

int PKI_X509_EST_DATA_set_x509_obj ( PKI_X509_EST_DATA *data, PKI_X509 *obj )
{
	PKI_MEM *mem = NULL;
	int ret = PKI_ERR;
	...
	ret = PKI_X509_EST_DATA_set_raw_data ( data, mem->data, (ssize_t) mem->size );
...
/*! \brief Sets the content of the EST_DATA via a EST_ISSUER_AND_SUBJECT */

int PKI_X509_EST_DATA_set_ias ( PKI_X509_EST_DATA *est_data, EST_ISSUER_AND_SUBJECT *ias )
{
	unsigned char *data = NULL;
	ssize_t size = 0;

	if ( !est_data || !est_data->value || !ias ) return PKI_ERR;

	if( (size = i2d_EST_ISSUER_AND_SUBJECT(ias, NULL)) <= 0 ) return PKI_ERR;

	if ((data = ( unsigned char * ) PKI_Malloc ( (size_t) size )) == NULL ) return PKI_ERR;

	if (i2d_EST_ISSUER_AND_SUBJECT( ias, &data ) <= 0 ) 
	{
		PKI_Free ( data );
		return PKI_ERR;
	}

	return PKI_X509_EST_DATA_set_raw_data ( est_data, data, size );
}

/*! \brief Sets the content of the EST_DATA (raw data) */
