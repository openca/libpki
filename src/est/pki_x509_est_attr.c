...
#define EST_CONF_LIST_SIZE	8

EST_CONF_ATTRIBUTE EST_ATTRIBUTE_list [EST_CONF_LIST_SIZE] = {
	{ EST_ATTRIBUTE_MESSAGE_TYPE, "2.16.840.1.113733.1.9.2", 
			"estMessageType", "EST Message Type", -1 },
	{ EST_ATTRIBUTE_PKI_STATUS, "2.16.840.1.113733.1.9.3", 
			"pkiStatus", "Status", -1 },
	{ EST_ATTRIBUTE_FAIL_INFO, "2.16.840.1.113733.1.9.4", 
			"failInfo", "Failure Info", -1 },
	{ EST_ATTRIBUTE_SENDER_NONCE, "2.16.840.1.113733.1.9.5", 
			"senderNonce", "Sender Nonce", -1 },
	{ EST_ATTRIBUTE_RECIPIENT_NONCE, "2.16.840.1.113733.1.9.6", 
			"recipientNonce", "Recipient Nonce", -1 },
	{ EST_ATTRIBUTE_TRANS_ID, "2.16.840.1.113733.1.9.7", 
			"transId", "Transaction Identifier", -1 },
	{ EST_ATTRIBUTE_EXTENSION_REQ, "2.16.840.1.113733.1.9.8", 
			"extensionReq", "Extension Request", -1 },
	{ EST_ATTRIBUTE_PROXY_AUTH, "1.3.6.1.4.1.4263.5.5", 
...
void PKI_X509_EST_init ( void ) {
	...
	EST_CONF_ATTRIBUTE *curr_oid = NULL;

        i = 0;
        while( i < EST_CONF_LIST_SIZE ) {
		curr_oid = &EST_ATTRIBUTE_list[i];
...
/*! \brief Returns the EST_ATTRIBUTE_TYPE from the attribute name */

EST_ATTRIBUTE_TYPE PKI_X509_EST_ATTRIBUTE_get_txt(const char * txt) {

	EST_CONF_ATTRIBUTE *curr = NULL;
	int i = 0;

    while( i < EST_CONF_LIST_SIZE ) {

    	// Gets the i-th attribute
		curr = &EST_ATTRIBUTE_list[i];
	...
	if (curr) return (EST_ATTRIBUTE_TYPE)curr->attr_type;

	// Not found
	return EST_ATTRIBUTE_TYPE_UNKNOWN;
}

/*! \brief Returns the PKI_ID of the specified EST_ATTRIBUTE_TYPE */
...
/*! \brief Returns the PKI_OID for the specified EST attribute */
...
PKI_OID *PKI_X509_EST_MSG_get_oid ( EST_ATTRIBUTE_TYPE est_attribute ) {
	...
	i = 0;
        while( i < EST_CONF_LIST_SIZE ) {
		curr = &EST_ATTRIBUTE_list[i];
		if ( curr->attr_type == est_attribute )
			break;
		i++;
	}
...
/*! \brief Sets the message type attribute in a EST message (signed P7) */
int PKI_X509_EST_MSG_set_attribute(PKI_X509_EST_MSG   * msg,
	...
	if ((id = PKI_X509_EST_ATTRIBUTE_get_nid ( type )) == NID_undef ) {
		return PKI_ERROR(PKI_ERR_EST_ATTRIBUTE_UNKNOWN, NULL);
	}
	...
	switch (type) {
		...
		case EST_ATTRIBUTE_MESSAGE_TYPE:
		case EST_ATTRIBUTE_PKI_STATUS:
		case EST_ATTRIBUTE_FAIL_INFO:
		case EST_ATTRIBUTE_TRANS_ID:
		case EST_ATTRIBUTE_EXTENSION_REQ:
		case EST_ATTRIBUTE_PROXY_AUTH: {
		...
		case EST_ATTRIBUTE_SENDER_NONCE:
		case EST_ATTRIBUTE_RECIPIENT_NONCE: {
			a = PKI_X509_ATTRIBUTE_VALUE_new(id,
				                       V_ASN1_OCTET_STRING,
									   data,
									   size);
...
/*! \brief Adds an attribute (identified by its name) to a EST message */
...
int PKI_X509_EST_MSG_set_attribute_by_name(PKI_X509_EST_MSG   * msg,
	...
	if ((type = PKI_X509_EST_ATTRIBUTE_get_txt(name)) == -1)
		return PKI_ERROR(PKI_ERR_EST_ATTRIBUTE_UNKNOWN, NULL);

	// Sets the Attribute in the Message
	return PKI_X509_EST_MSG_set_attribute(msg, type, data, size);
...
int PKI_X509_EST_MSG_set_attribute_int(PKI_X509_EST_MSG * msg,
	...
	return PKI_X509_EST_MSG_set_attribute(msg, id,
...
PKI_MEM * PKI_X509_EST_MSG_get_attr_value(const PKI_X509_EST_MSG * const msg,
	...
	if ((nid = PKI_X509_EST_ATTRIBUTE_get_nid(type)) == NID_undef) {
		PKI_ERROR(PKI_ERR_EST_ATTRIBUTE_UNKNOWN, NULL);
		return NULL;
	}
...
int PKI_X509_EST_MSG_get_attr_value_int(const PKI_X509_EST_MSG * const msg,
	...
	if ((mem = PKI_X509_EST_MSG_get_attr_value(msg, type)) == NULL) {
...
/*! \brief Generates a new PKI_MEM suitable for the transId of a EST message */
...
/*! \brief Sets the transactionId attribute in a EST message */
...
int PKI_X509_EST_MSG_set_trans_id(PKI_X509_EST_MSG * msg,
	...
	// Sets the Attribute
	ret = PKI_X509_EST_MSG_set_attribute(msg,
...
char * PKI_X509_EST_MSG_get_trans_id(const PKI_X509_EST_MSG * const msg) {
	...
	if((mem = PKI_X509_EST_MSG_get_attr_value(msg,
						EST_ATTRIBUTE_TRANS_ID)) == NULL) {
		return NULL;
	}
...
/*! \brief Sets the senderNonce attribute in a EST message */
...
/*! \brief Sets the recipientNonce attribute from a EST message */
...
EST_MESSAGE_TYPE PKI_X509_EST_MSG_get_type(const PKI_X509_EST_MSG * const msg) {
	...
	return PKI_X509_EST_MSG_get_attr_value_int(msg,
			                                    EST_ATTRIBUTE_MESSAGE_TYPE);
}
...
EST_STATUS PKI_X509_EST_MSG_get_status(const PKI_X509_EST_MSG * const msg) {
	...
	return (EST_STATUS) PKI_X509_EST_MSG_get_attr_value_int(msg,
			                                                  EST_ATTRIBUTE_PKI_STATUS);
}
...
EST_FAILURE PKI_X509_EST_MSG_get_failinfo(const PKI_X509_EST_MSG * const msg) {
	...
	return (EST_FAILURE) PKI_X509_EST_MSG_get_attr_value_int(msg,
				                                               EST_ATTRIBUTE_FAIL_INFO);
}
...
PKI_MEM *PKI_X509_EST_MSG_get_sender_nonce(const PKI_X509_EST_MSG * const msg) {
	...
	return PKI_X509_EST_MSG_get_attr_value(msg,
				                            EST_ATTRIBUTE_SENDER_NONCE);
}
