...
PKI_SCHEME_ID PKI_X509_KEYPAIR_VALUE_get_scheme(const PKI_X509_KEYPAIR_VALUE *pVal) {
	...
	pkey_type = EVP_PKEY_type(PKI_X509_KEYPAIR_VALUE_get_id(pVal));
...
int PKI_X509_KEYPAIR_VALUE_is_digest_supported(const PKI_X509_KEYPAIR_VALUE * pkey, const PKI_DIGEST_ALG * digest) {
	...
	int pkey_type = EVP_PKEY_type(PKI_X509_KEYPAIR_VALUE_get_id(pkey));
...
PKI_MEM * PKI_X509_KEYPAIR_VALUE_encrypt(const PKI_X509_KEYPAIR_VALUE * pVal, 
	...
	if (flags <= 0) {
		...
		int pkey_id = EVP_PKEY_type(PKI_X509_KEYPAIR_VALUE_get_id(pVal));
	...
	if (padding > 0) {
		...
		int pkey_id = EVP_PKEY_type(PKI_X509_KEYPAIR_VALUE_get_id(pkey));
